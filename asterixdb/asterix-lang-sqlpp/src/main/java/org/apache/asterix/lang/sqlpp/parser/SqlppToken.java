...
public abstract class SqlppToken implements Serializable {
    ...
    public List<SqlppHint> hints = new LinkedList<>();
    public List<String> hintParams = new LinkedList<>();
    ...
    public boolean parseHint(String text) {
        ...
        SqlppHint hint = SqlppHint.findByIdentifier(id);
        if (hint != null) {
            hints.add(hint);
            hintParams.add(paramStart >= 0 ? text.substring(paramStart).trim() : null);
            return true;
        } else {
            hintParams.add(text);
            return false;

...
public class JobBuilder implements IHyracksJobBuilder {
    ...
    private Map<ILogicalOperator, ArrayList<ILogicalOperator>> tempToJoinBlockingMap = new HashMap<>();

    private Map<ILogicalOperator, ArrayList<ILogicalOperator>> seqBuildBlockedToBlockers = new HashMap<>();
    ...
    @Override
    public void contributeTempJoinResultControlDependency(ILogicalOperator temp, ArrayList<ILogicalOperator> joins) {
        ArrayList<ILogicalOperator> blockedJoins = tempToJoinBlockingMap.get(temp);
        if (blockedJoins == null) {
            blockedJoins = new ArrayList<>();
            tempToJoinBlockingMap.put(temp, blockedJoins);
        }
        blockedJoins.addAll(joins);
    }

    public void contributeSeqBuildControlDependency(ILogicalOperator blocked, ILogicalOperator blocker) {
        ArrayList<ILogicalOperator> blockers = seqBuildBlockedToBlockers.get(blocked);
        if (blockers == null) {
            blockers = new ArrayList<>();
            seqBuildBlockedToBlockers.put(blocked, blockers);
        }
    ...
    public void buildSpec(List<ILogicalOperator> roots) throws AlgebricksException {
        ...
        addTempToNextJoinContorlDependency();
        addSeqBuildControlDependencies();
    }
    ...
    private void addTempToNextJoinContorlDependency() throws AlgebricksException {
        for (Map.Entry<ILogicalOperator, ArrayList<ILogicalOperator>> entry : tempToJoinBlockingMap.entrySet()) {
            IOperatorDescriptor temp = findOpDescForAlgebraicOp(entry.getKey());
            ArrayList<IOperatorDescriptor> joinsDesc = new ArrayList<>();
            for (ILogicalOperator blocker : entry.getValue()) {
                joinsDesc.add(findOpDescForAlgebraicOp(blocker));
            }
            jobSpec.addTempToNextJoinsControlDependency(temp, joinsDesc);
        }
    }

    private void addSeqBuildControlDependencies() throws AlgebricksException {
        for (Map.Entry<ILogicalOperator, ArrayList<ILogicalOperator>> entry : seqBuildBlockedToBlockers.entrySet()) {
            IOperatorDescriptor blocked = findOpDescForAlgebraicOp(entry.getKey());
            ArrayList<IOperatorDescriptor> blockerDescs = new ArrayList<>();
            for (ILogicalOperator blocker : entry.getValue()) {
                blockerDescs.add(findOpDescForAlgebraicOp(blocker));
            }
            jobSpec.addSeqBuildControlDependency(blocked, blockerDescs);
        }
    }

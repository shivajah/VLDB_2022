...
public class PlanCompiler {
    ...
    public PlanCompiler(JobGenContext context, boolean seqBuild) {
        this.context = context;
        this.seqBuild = seqBuild;
    }
    ...
    private JobSpecification compilePlanImpl(ILogicalPlan plan, boolean isNestedPlan, IOperatorSchema outerPlanSchema,
        ...
        for (Mutable<ILogicalOperator> opRef : plan.getRoots()) {
            ...
            ArrayList<ILogicalOperator> tempJoins = new ArrayList<>();

            addTempToNextJoinBlockers(opRef, builder, tempJoins);
            if (seqBuild) {
                List<ILogicalOperator> joins = new LinkedList<>();
                addSequentialBlocks(opRef, builder, joins);
            }
        }
    ...
    private void addTempToNextJoinBlockers(Mutable<ILogicalOperator> opRef, IHyracksJobBuilder builder,
            ArrayList<ILogicalOperator> joins) {
        ILogicalOperator op = opRef.getValue();
        if (op instanceof TempOperator) {
            builder.contributeTempJoinResultControlDependency(op, joins);
            joins.clear();
        } else if (op instanceof AbstractBinaryJoinOperator) {
            joins.add(op);
        }
        for (Mutable<ILogicalOperator> opChild : op.getInputs()) {
            addTempToNextJoinBlockers(opChild, builder, joins);
        }
    }

    private void addSequentialBlocks(Mutable<ILogicalOperator> opRef, IHyracksJobBuilder builder,
            List<ILogicalOperator> joins) {
        ILogicalOperator op = opRef.getValue();
        if (op instanceof AbstractBinaryJoinOperator) {
            joins.add(op);
        }
        for (Mutable<ILogicalOperator> opChild : op.getInputs()) {
            addSequentialBlocks(opChild, builder, joins);
        }
        if (op instanceof AbstractBinaryJoinOperator) {
            int index = joins.indexOf(op);
            if (index + 1 < joins.size()) {
                builder.contributeSeqBuildControlDependency(op, joins.get(index + 1));
            }
        }
    }

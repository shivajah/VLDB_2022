...
public class AbstractFrameAppender implements IFrameAppender {
    ...
    public int getRemainingSpaceAfterInsertion(int fieldCount, int tupleLength) {
        if (hasEnoughSpace(fieldCount, tupleLength)) {
            return FrameHelper.getTupleCountOffset(frame.getFrameSize()) - (tupleDataEndOffset
                    + FrameHelper.calcRequiredSpace(fieldCount, tupleLength) + tupleCount * FrameConstants.SIZE_LEN);
        }
        return -1;
    }

    public double getFrameFullness() {
        return 100 * (1 - (FrameHelper.getTupleCountOffset(frame.getFrameSize())
                - (tupleDataEndOffset + tupleCount * FrameConstants.SIZE_LEN)) * 1.0 / frame.getFrameSize());
    }

    public int getFreeSpaceAmount() {
        return FrameHelper.getTupleCountOffset(frame.getFrameSize())
    ...
    public boolean canHoldNewTuple(int fieldCount, int dataLength) throws HyracksDataException {

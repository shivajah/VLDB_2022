...
public class JobActivityGraphBuilder implements IActivityGraphBuilder {
    ...
    private Map<IOperatorDescriptor, ArrayList<ActivityId>> operatorActivityMap;
    ...
    public JobActivityGraphBuilder(JobSpecification jobSpec, Set<JobFlag> jobFlags) {
        ...
        operatorActivityMap = new HashMap<>();
    ...
    public void addActivity(IOperatorDescriptor op, IActivity task) {
        ...
        if (!operatorActivityMap.containsKey(op)) {
            ArrayList<ActivityId> activities = new ArrayList<>();
            operatorActivityMap.put(op, activities);
        }
        operatorActivityMap.get(op).add(task.getActivityId());
        ActivityId activityId = task.getActivityId();
        jag.getActivityMap().put(activityId, task);
    }

    private void addTempToNextJoinsControlDependency() {

        for (Map.Entry<IOperatorDescriptor, ArrayList<IOperatorDescriptor>> entry : jobSpec.getTempToJoinBlockingMap()
                .entrySet()) {
            ActivityId tempWrite = operatorActivityMap.get(entry.getKey()).get(0);
            ActivityId tempRead = operatorActivityMap.get(entry.getKey()).get(1);
            ArrayList<ActivityId> joinBuilds = new ArrayList<>();
            for (IOperatorDescriptor join : entry.getValue()) {
                joinBuilds.add(operatorActivityMap.get(join).get(0));
            }
            for (ActivityId build : joinBuilds) {
                addToValueSet(jag.getBlocked2BlockerMap(), build, tempWrite);
                addToValueSet(jag.getBlocked2BlockerMap(), tempRead, build);
            }
        }
    }

    private void addSeqBuildControlDependency() {

        for (Map.Entry<IOperatorDescriptor, ArrayList<IOperatorDescriptor>> entry : jobSpec
                .getSeqBuildBlockedToBlockers().entrySet()) {
            ActivityId blocked = operatorActivityMap.get(entry.getKey()).get(0);
            ArrayList<ActivityId> blockerActivities = new ArrayList<>();
            for (IOperatorDescriptor blocker : entry.getValue()) {
                blockerActivities.add(operatorActivityMap.get(blocker).get(0));
            }
            for (ActivityId blocker : blockerActivities) {
                addToValueSet(jag.getBlocked2BlockerMap(), blocked, blocker);
            }
        }
    }

    public void finish() {
        addTempToNextJoinsControlDependency();
        addSeqBuildControlDependency();
